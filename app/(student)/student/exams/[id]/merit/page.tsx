...
import { getStudentExamMeritList } from "@/features/exams/queries";

type StudentExamMeritPageProps = {
  params: Promise<{
    id: string;
  }>;
};

export default async function StudentExamMeritPage({
  params,
}: StudentExamMeritPageProps) {
  const { id } = await params;
  const meritList = await getStudentExamMeritList(id);
      ...
      <div className="mx-auto max-w-5xl">
        <header className="flex flex-col gap-4 border-b border-[#d8dfda] pb-6 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Link
            ...
            href={`/student/exams/${meritList.exam.id}`}
          >
            Exam
          </Link>
        </header>
